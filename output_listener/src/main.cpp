...
#include "output_listener/bresenham.hpp"
#include "output_listener/util.hpp"
...
public:
  // Constructor
  OutputListenerNode(ros::NodeHandle& node);
  // Destructor
...
// Constructor
OutputListenerNode::OutputListenerNode(ros::NodeHandle& node)
{
                                    ...
                                    SaveOutput::Response& response)
{
  ...
  if (!fs::exists(metricsFileDir)) {
    ROS_INFO("Directory %s does not exist", metricsFileDir.c_str());
    return true;
  }

  if (!fs::exists(gridMapFileDir)) {
    ROS_INFO("Directory %s does not exist", gridMapFileDir.c_str());
    return true;
  }

  // Save the metrics as JSON
  if (!this->SaveMetrics(request.metrics_file_name)) {
    ROS_INFO("Failed to save the metrics to %s",
  ...
  if (!this->mSrvGridMap.call(srvOccupancyGrid)) {
    ROS_INFO("Failed to call the service %s",
  ...
  if (occupancyGrids.size() != resolutionLevels.size()) {
    ROS_INFO("Number of the occupancy grids is different from the number of "
  ...
  for (std::size_t i = 0; i < occupancyGrids.size(); ++i) {
    ...
    if (!this->SaveMap(baseFileName, occupancyGrids[i],
      ...
      ROS_INFO("Failed to save the occupancy grid (resolution level: %d) "
               "to the directory %s",
...
bool OutputListenerNode::SaveMetrics(const std::string& fileName) const
{
  ...
  try {
    ...
    ROS_INFO("Failed to save the metrics to %s, message: %s",
...
{
  // Compute the size of the cropped occupancy grid
  const Eigen::AlignedBox2i boundingBox = this->GetBoundingBox(occupancyGrid);
  ...
  if (!this->DrawMap(mapImageView, occupancyGrid, boundingBox)) {
    ROS_INFO("Failed to create an image from the occupancy grid");
    return false;
  }
  ...
  if (!this->DrawTrajectory(mapImageView, trajectory, origin, resolution)) {
    ROS_INFO("Failed to draw robot trajectory on the occupancy grid image");
    return false;
  }
  ...
  try {
    ...
    ROS_INFO("Failed to save the occupancy grid image to %s, message: %s",
             fileName.c_str(), e.what());
    return false;
  }
  ...
  try {
    ...
    ROS_INFO("Failed to save the metadata for the occupancy grid to %s, "
             "message: %s",
  ...
  const Eigen::AlignedBox2i& boundingBox) const
{
  ...
  if (image.width() != boundingBox.sizes().x() ||
    ...
    ROS_INFO("Image size is inconsistent with the bounding box: "
  ...
  const float resolution) const
{
  ...
  const gil::rgb8_pixel_t kLineColor { 255, 0, 0 };
  ...
  for (std::size_t i = 1; i < trajectory.size(); ++i) {
    ...
    for (const auto& interpolatedIdx : indices) {
      if (interpolatedIdx.x() < 0 ||
          interpolatedIdx.x() > image.width() - kLineWidth ||
          interpolatedIdx.y() < 0 ||
          interpolatedIdx.y() > image.height() - kLineWidth)
        continue;

      const auto& subView = gil::subimage_view(
        image, interpolatedIdx.x(), interpolatedIdx.y(),
        kLineWidth, kLineWidth);
      gil::fill_pixels(subView, kLineColor);
    }
...
int main(int argc, char** argv)
{
  ...
  OutputListenerNode outputListener { nodeHandle };
